...
```text
เรานับจำนวนของจำนวนนับ ทั้งหมดที่อยู่ในช่วงปิดตั้งแต่ A ถึง B (นับรวมค่า A และ B ด้วย) โดยมีเงื่อนไขว่าเมื่อนำ แต่ละตัวไปหารด้วยตัวหาร d แล้ว จะต้องได้เศษเหลือเท่า
...
Output:
 จํานวนข้อมูลที่สอดคล้อง total :

Constraints:
ขอบเขตการนับ: เป็นช่วงแบบรวมขอบ [A, B] ทำให้ต้องตรวจตั้งแต่ค่า A ไปจนถึง Bขอบเขตของเศษ: เศษ r จะมีค่าน้อยกว่าตัวหาร d เสมอ (r < d)
```
...
```text
Step 1: อ่านค่าจำนวนนับ A, B, d, r ตามลำดับจาก input แล้วแปลงเป็นจำนวนเต็ม (int)
Step 2: สร้างตัวแปรตัวนับ (total = 0) เพื่อเก็บจำนวนตัวเลขที่ตรงตามเงื่อนไข
Step 3: ใช้วงวน for วนลูปตรวจสอบตัวเลข i ตั้งแต่ A ถึง  B (ใช้ range(A, B + 1))
Step 4: ในแต่ละรอบ เช็กว่า i % d == r หรือไม่
Step 5: ถ้าเป็นจริง ให้บวกค่า total เพิ่มขึ้น 1 (total += 1)
Step 6: เมื่อวนลูปครบแล้ว ให้พิมพ์ค่า total ออกมาเป็นผลลัพธ์
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก วงวน for วนลูปตรวจสอบตัวเลขทีละตัวตั้งแต่ A ถึง B โดยใช้ range(A, B + 1) แล้วนำตัวเลข i ในแต่ละรอบมาตรวจสอบเศษด้วยตัวดำเนินดำเนินการ modulo (i % d == r) หากเงื่อนไขเป็นจริงจะทำการเพิ่มค่าตัวนับ total ทีละ 1 แล้วจึงแสดงผลลัพธ์รวมออกมา
```
...
```text
กรณีไม่มีตัวเลขใดสอดคล้องกับเงื่อนไขเลย
```
